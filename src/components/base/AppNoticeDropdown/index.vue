    ...
    div(class="notice__panel")
      div(class="notice__tabs")
        button(
          ...
          span 未讀
          span(v-if="unreadTotal > 0" class="notice__count") {{ unreadTotal > 99 ? "99+" : unreadTotal }}
        button(
          type="button"
      ...
      div(v-if="list.length > 0" class="notice__list")
        div(
          ...
          AppDictTag(v-model="item.type" code="notice_type" size="small")
          el-text(size="small" class="notice__title" truncated @click="read(item.id)") {{ item.title }}
          div(class="notice__time") {{ item.publishTime }}
